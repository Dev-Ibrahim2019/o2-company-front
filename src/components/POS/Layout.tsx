// src/components/POS/Layout.tsx
//
// يستخدم AuthContext بدل useApp() لبيانات المستخدم وتسجيل الخروج
...
export const POSLayout: React.FC<{
  ...
  // ✅ نستخدم AuthContext بدل useApp() لبيانات المستخدم
  ...
  const getRoleLabel = () => {
    ...
    if (isBranchManager)
      return `مدير: ${user?.branch?.name ?? ""}`;
    if (isDeptStaff) return "موظف قسم";
    if (isAggregator) return "مجمع طلبات";
    if (isFinance) return "قسم المالية";
