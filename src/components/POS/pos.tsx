...
// 1. branchId يُقرأ من AuthContext (المستخدم الحقيقي) بدل القيمة الوهمية
// 2. currentUser يأتي من AuthContext
// 3. userRole يأتي من AuthContext
...
// نستورد useApp فقط للبيانات التي لم تُنقل بعد للـ API
import { useApp } from "../../../store";

export const POS: React.FC<{ onViewTables: () => void }> = ({
  onViewTables,
}) => {
  // ── Auth (الحقيقي) ────────────────────────────────────────────────────────
  ...
  // ── الدور والفرع من المستخدم الحقيقي ─────────────────────────────────────
  const userRole = authUser?.role ?? "EMPLOYEE";
  const isHospitality = userRole === "HOSPITALITY";

  // ✅ branchId من AuthContext بدل القيمة الوهمية
  const branchId: number = authUser?.branch_id ?? authUser?.branchId ?? 1;
  ...
  const handleQuickTotalChange = (totalStr: string) => {
    ...
    if (item && totalStr)
      setQuickQty(
        ((parseFloat(totalStr) || 0) / item.price).toFixed(2)
      );
  };
  ...
  const submitOrder = async (
    ...
    const shouldConfirm =
      status === OrderStatus.CONFIRMED || isHospitality;
    ...
    // ✅ cashier_id من المستخدم الحقيقي
    const cashierId = authUser?.employee_id
