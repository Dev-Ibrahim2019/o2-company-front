...
import { getStatusColor, getStatusLabel } from "../administration/EmployeeManagement/utils";
...
const unavailable = "غير متاح حاليًا";
...
function EmployeeRow({ employee, onSelect, onEdit, onDelete }: { employee: EmployeeFromApi; onSelect: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <tr onClick={onSelect} onKeyDown={handleKeyDown} tabIndex={0} role="button" aria-label={`عرض تفاصيل الموظف ${employee.name}`} className="cursor-pointer transition hover:bg-white/[0.035] focus:bg-white/[0.05] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-red-500/70">
      ...
      <td className="px-4 py-4">{isLate ? <span className="inline-flex items-center gap-1 rounded-lg border border-red-500/30 bg-red-500/10 px-2 py-1 text-[10px] font-black text-red-300"><AlertTriangle size={12} />تأخير يتجاوز 15 دقيقة</span> : performance.alert ? <span className="text-[10px] font-bold text-amber-300">{String(performance.alert)}</span> : <span className="text-[10px] font-bold text-slate-600">لا تنبيهات</span>}</td>
...
function EmployeeFormModal({ employee, branches, departments, jobTitles, saving, onSave, onClose }: { employee: EmployeeFromApi | null; branches: Branch[]; departments: Department[]; jobTitles: JobTitle[]; saving: boolean; onSave: (data: EmployeeFormData) => Promise<void>; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[130] flex items-center justify-center bg-slate-950/80 p-3 backdrop-blur-sm" dir="rtl" onMouseDown={onClose}><div onMouseDown={(event) => event.stopPropagation()} className="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-2xl">
    <div className="flex items-center justify-between border-b border-white/10 bg-slate-900/70 p-5"><div><h2 className="text-lg font-black text-white">{employee ? "تعديل ملف الموظف" : "إضافة موظف عمليات"}</h2><p className="mt-1 text-[10px] font-bold text-slate-500">بيانات الموارد البشرية والدور التشغيلي</p></div><button onClick={onClose} className="rounded-xl p-2 text-slate-500 hover:bg-white/5 hover:text-white"><X size={18} /></button></div>
    ...
    <div className="flex justify-end gap-2 border-t border-white/10 bg-slate-900/60 p-4"><button onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs font-black text-slate-400 hover:bg-white/5 hover:text-white">إلغاء</button><button disabled={!valid || saving} onClick={() => onSave(form)} className="flex items-center gap-2 rounded-xl bg-red-600 px-5 py-2.5 text-xs font-black text-white hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-40">{saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}{saving ? "جاري الحفظ" : "حفظ الموظف"}</button></div>
  </div></div>;
}
