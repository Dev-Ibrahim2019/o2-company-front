  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-white">الشكاوى والمتابعة</h2>
          <p className="text-xs text-slate-400">إدارة شكاوى العملاء ومتابعتها</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {tab === "details" ? (
            <>
              ...
              <p className="text-xs text-slate-400">{complaint.description || "—"}</p>
              <div className="text-xs text-slate-400 space-y-1">
                <p>النوع: {complaintTypes.find(t => t.value === complaint.type)?.label || complaint.type}</p>
                <p>الحساسية: {complaint.is_sensitive ? "حساسة" : "عادية"}</p>
                {complaint.customer && <p>العميل: {complaint.customer.name}</p>}
                {complaint.order && <p>الطلب: {complaint.order.order_number}</p>}
            ...
            <>
              {loadingTimeline ? <div className="flex justify-center py-8"><Loader2 size={20} className="animate-spin text-red-500" /></div>
                        ...
                        <div className="flex-1 pb-3">
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                            ...
                            {f.old_status && f.new_status && (
                              <span>({f.old_status} → {f.new_status})</span>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                ) : <p className="text-center text-slate-500 text-xs">لا توجد متابعات</p>}

              <div className="border-t border-white/5 pt-3 mt-3">
                <textarea value={followupNotes} onChange={e => setFollowupNotes(e.target.value)} placeholder="أضف متابعة..."
                ...
                <button onClick={handleAddFollowup} disabled={sending || !followupNotes.trim()}
                  ...
                  {sending ? "جاري الإرسال..." : "إضافة متابعة"}
                </button>
              </div>
            </>
          )}
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" dir="rtl">
      ...
      <div className="relative bg-slate-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-md p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-white font-black text-sm">شكوى جديدة</h3>
          <button onClick={onClose} className="p-1 hover:bg-white/5 rounded-lg"><X size={16} className="text-slate-400" /></button>
        </div>
        <input value={customerId} onChange={e => setCustomerId(e.target.value)} placeholder="رقم العميل *" type="number"
        ...
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="عنوان الشكوى *"
          className="w-full bg-slate-800 border border-white/10 rounded-xl p-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-red-500/50" />
        ...
        <input value={orderId} onChange={e => setOrderId(e.target.value)} placeholder="رقم الطلب (اختياري)" type="number"
          className="w-full bg-slate-800 border border-white/10 rounded-xl p-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-red-500/50" />
        ...
        <button onClick={handleSubmit} disabled={sending || !title.trim() || !customerId}
          ...
          {sending ? "جاري الإنشاء..." : "إنشاء الشكوى"}
        </button>
      </div>
    </div>
  );
...
const ComplaintPriorityBadge: React.FC<{ priority: string }> = ({ priority }) => {
  const map: Record<string, { color: string; label: string }> = {
    ...
    critical: { color: "text-red-400 bg-red-500/10", label: "حرجة" },
  };
