...
export const CallCenterPOS: React.FC = () => {
  ...
  const [wrapUpTicketId, setWrapUpTicketId] = useState<number | null>(null);
  ...
  const endCall = async () => {
    const activeTicketId = ticket.ticket?.id;
    try {
      if (ticket.ticket && !success) await ticket.complete("call_ended", note);
      if (activeTicketId) setWrapUpTicketId(activeTicketId);
  ...
  const newCallerActive =
    ...
    calls.phase === "waiting";
  return <div className="relative flex h-full min-h-0 flex-col bg-slate-50 font-['Tajawal']" dir="rtl">
    ...
    {!customerPaneOpen&&customer&&<button onClick={()=>setCustomerPaneOpen(true)} className="absolute left-5 top-4 z-40 hidden min-h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 shadow-md xl:flex"><Users size={15}/>فتح ملف العميل</button>}
    {!branchId&&<div className="border-b border-amber-200 bg-amber-50 p-2 text-center text-xs text-amber-700"><label className="font-bold">اختر الفرع المسؤول عن تنفيذ الطلب <select value={branchId} onChange={e=>setBranchId(Number(e.target.value))} className="mr-2 min-h-10 rounded-md border border-amber-200 bg-white px-3 text-slate-900"><option value={0}>اختر الفرع</option>{branches.map(branch=><option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label></div>}
    <div className="flex min-h-0 flex-1 gap-4 overflow-hidden bg-slate-50 p-2 sm:p-4">
      {customerPaneOpen&&customer&&<div className="relative hidden min-h-0 w-[320px] shrink-0 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm xl:flex 2xl:w-[380px]">
        <button onClick={()=>setCustomerPaneOpen(false)} aria-label="إغلاق ملف العميل" className="absolute left-2 top-2 z-20 flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-slate-100 text-slate-600 hover:bg-slate-200"><span aria-hidden="true">×</span></button>
    ...
    {customerPaneOpen&&customer&&<div className="fixed inset-0 z-[65] hidden items-stretch bg-slate-900/30 md:flex xl:hidden" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setCustomerPaneOpen(false);}}>
      <aside role="dialog" aria-modal="true" aria-label="ملف العميل" className="relative mr-auto flex h-full w-[360px] max-w-[92vw] overflow-hidden border-r border-slate-200 bg-white shadow-2xl">
        <button autoFocus onClick={()=>setCustomerPaneOpen(false)} aria-label="إغلاق ملف العميل" className="absolute left-3 top-3 z-20 flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 text-slate-600 hover:bg-slate-200 focus:ring-2 focus:ring-red-300"><span aria-hidden="true">×</span></button>
    ...
    {wrapUpTicketId&&<PostCallWrapUpModal ticketId={wrapUpTicketId} onDone={()=>setWrapUpTicketId(null)}/>}
    ...
    {calls.phase === "identifying" && resolution.status === "searching" && <div className="pointer-events-none absolute inset-0 z-[75] bg-slate-900/10"><CustomerSearchLoading phone={calls.session?.callerNumber||""}/></div>}
...
const CallSimulator:React.FC<{isBreak:boolean;onBreak:()=>void;onSimulate:(p:string)=>void}>=({isBreak,onBreak,onSimulate})=>{
  ...
  return <div className="absolute bottom-4 left-4 z-50" dir="rtl">{open&&<div className="mb-2 w-72 rounded-xl border border-slate-200 bg-white p-3 text-slate-900 shadow-xl"><label className="text-xs font-bold">محاكاة مكالمة<input value={phone} onChange={e=>setPhone(e.target.value)} dir="ltr" className="mt-2 min-h-10 w-full rounded-lg border border-slate-200 bg-slate-50 px-3"/></label><button disabled={!valid||isBreak} onClick={()=>{localStorage.setItem("call_center_simulation_phone",phone);onSimulate(phone);setOpen(false);}} className="mt-2 min-h-10 w-full rounded-lg bg-[#E20004] text-xs font-black text-white disabled:opacity-50">بدء المكالمة</button><button onClick={onBreak} className="mt-2 min-h-9 w-full text-xs text-slate-500">{isBreak?"إنهاء الاستراحة":"بدء استراحة"}</button></div>}<button onClick={()=>setOpen(value=>!value)} aria-label="أدوات اختبار المكالمات" className="flex h-11 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600 shadow-md"><Headphones size={16}/>اختبار مكالمة</button></div>;
};

const Incoming:React.FC<{phone:string;loading:boolean;ticketReady:boolean;ticketError:string|null;onRetryTicket:()=>void;onAnswer:()=>void;onReject:()=>void;onTransfer:(x:string)=>void}>=({phone,loading,ticketReady,ticketError,onRetryTicket,onAnswer,onReject,onTransfer})=><section role="dialog" aria-modal="false" aria-label="مكالمة واردة" className="pointer-events-auto w-full max-w-lg rounded-2xl border border-green-200 bg-white p-4 text-center text-slate-900 shadow-2xl sm:p-5" dir="rtl"><div className="flex items-center justify-center gap-3"><PhoneIncoming className="animate-pulse text-green-600" size={32}/><div className="text-right"><p className="text-xs text-slate-400">مكالمة واردة</p><h1 className="text-xl font-black" dir="ltr">{phone}</h1></div></div>{ticketError&&<div className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-2 text-xs text-rose-700"><p>{ticketError}</p><button onClick={onRetryTicket} className="mt-2 min-h-10 rounded-md border border-rose-200 px-4 font-bold">إعادة محاولة إنشاء التذكرة</button></div>}<div className="mt-4 grid grid-cols-2 gap-2"><button disabled={loading||!ticketReady} onClick={onAnswer} className="min-h-11 rounded-lg bg-green-600 font-black text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50">{loading?<Loader2 className="mx-auto animate-spin"/>:<><Phone size={18} className="ml-2 inline"/>رد</>}</button><button onClick={onReject} className="min-h-11 rounded-lg bg-rose-600 font-black text-white"><PhoneOff size={18} className="ml-2 inline"/>رفض</button></div>{!ticketReady&&!ticketError&&<p className="mt-2 text-xs text-slate-400">جارٍ تجهيز تذكرة المكالمة…</p>}<div className="mt-3 flex justify-center gap-2 overflow-x-auto">{DEFAULT_EXTENSIONS.slice(0,3).map(ext=><button key={ext.extension} onClick={()=>onTransfer(ext.extension)} className="min-h-10 shrink-0 rounded-lg border border-slate-200 px-3 text-xs text-slate-700"><PhoneForwarded size={14} className="ml-1 inline"/>{ext.name}</button>)}</div></section>;

const CustomerSearchLoading=({phone}:{phone:string})=><div className="flex h-full items-start justify-center p-5 pt-24 text-slate-900" dir="rtl"><div className="pointer-events-auto rounded-xl border border-red-200 bg-white px-8 py-5 text-center shadow-xl"><Loader2 className="mx-auto mb-3 animate-spin text-[#E20004]" size={34}/><h1 className="text-base font-black">جارٍ التحقق من بيانات العميل...</h1><p className="mt-2 text-xs text-slate-400" dir="ltr">{phone}</p><p className="mt-2 text-[11px] text-slate-400">يمكنك متابعة إضافة الأصناف أثناء البحث.</p></div></div>;
const CustomerSearchError=({message,onRetry,onEnd}:{message:string|null;onRetry:()=>void;onEnd:()=>void})=><section role="alertdialog" aria-modal="false" className="w-full max-w-md rounded-xl border border-rose-200 bg-white p-5 text-center text-slate-900 shadow-xl" dir="rtl"><AlertTriangle className="mx-auto text-rose-500" size={34}/><h1 className="mt-3 text-lg font-black">تعذر التحقق من بيانات العميل</h1><p className="mt-2 text-sm text-rose-600">{message}</p><div className="mt-4 grid grid-cols-2 gap-2"><button autoFocus onClick={onRetry} className="min-h-11 rounded-lg bg-[#E20004] font-black text-white">إعادة المحاولة</button><button onClick={onEnd} className="min-h-11 rounded-lg border border-rose-200 text-xs text-rose-600">إنهاء المكالمة</button></div></section>;
const CustomerCandidateSelector=({candidates,onSelect,onRetry}:{candidates:CustomerSearchResult[];onSelect:(v:CustomerSearchResult)=>void;onRetry:()=>void})=><section role="dialog" aria-modal="false" aria-label="اختيار العميل" className="max-h-[70vh] w-full max-w-xl overflow-y-auto rounded-xl border border-slate-200 bg-white p-5 text-slate-900 shadow-xl" dir="rtl"><div className="mb-4 flex items-center gap-2"><Users className="text-blue-500"/><div><h1 className="font-black">اختر العميل الصحيح</h1><p className="text-xs text-slate-400">يمكن متابعة تجهيز الفاتورة خلف هذه البطاقة.</p></div></div><div className="space-y-2">{candidates.map((value,index)=><button autoFocus={index===0} key={value.id} onClick={()=>onSelect(value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 p-3 text-right hover:border-red-300 focus:ring-2 focus:ring-red-300"><b>{value.name}</b><span className="mt-1 block text-xs text-slate-400" dir="ltr">{value.phone||value.mobile} · {value.code}</span></button>)}</div><button onClick={onRetry} className="mt-3 min-h-10 w-full text-xs text-blue-600">إعادة البحث</button></section>;
