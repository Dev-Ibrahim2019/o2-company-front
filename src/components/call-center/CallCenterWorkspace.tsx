...
import {
  AlertTriangle, Banknote, Check, CheckCircle, ChevronLeft, CircleDollarSign, Clock3, CreditCard,
  FileText, MapPin, Minus, PackageSearch, Phone, PhoneForwarded, Plus, Search, ShoppingBag, Wallet, Zap,
...
export type WorkspaceTab = "products" | "order" | "customer" | "cart";
export const activeOrderScopeLabels: Record<ActiveOrderScope, string> = {
  awaiting_payment: "بانتظار الدفع",
  kitchen_active: "تم الدفع · قيد التحضير",
  delivery_active: "تم الجمع · مع الديلفري",
  fulfilled_recent: "تم التسليم",
};
...
export const CallCenterCart: React.FC<{
  ...
  return <aside className="flex min-h-0 w-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-900 text-slate-50 shadow-2xl sm:rounded-[2rem]">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      <section className="border-t border-[#2A3039] p-3">
        <button type="button" onClick={()=>window.dispatchEvent(new Event("call-center:manage-payment"))} className="min-h-11 w-full rounded-lg border border-red-600/30 bg-red-600/10 text-xs font-black text-red-300 hover:bg-red-600/20 focus:ring-2 focus:ring-red-600/50">
          إدارة بيانات العميل والعنوان وطرق الدفع
        </button>
        <div className="mt-2 flex items-center justify-between text-xs text-slate-400"><span>{payments.length} دفعات</span><strong dir="ltr">{money(paid)} / {money(total)}</strong></div>
      </section>
    </div>
