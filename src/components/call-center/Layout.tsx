    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden text-slate-100 font-['Tajawal']" dir="rtl">
      ...
      <aside className={`fixed top-0 right-0 h-full bg-slate-900 border-l border-white/5 flex flex-col p-4 shadow-2xl transition-all duration-300 z-50 ${isSidebarOpen ? "w-64 translate-x-0" : "w-64 translate-x-full lg:w-20 lg:translate-x-0"}`}>
        <div className={`mb-8 flex items-center gap-3 ${!isSidebarOpen ? "justify-center" : "px-4"}`}>
          <div className="w-10 h-10 bg-cyan-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-cyan-900/20 shrink-0">
            <Headphones size={24} />
          </div>
          {isSidebarOpen && (
            <div>
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)}
          className="absolute -left-3 top-20 w-6 h-6 bg-cyan-600 rounded-full hidden lg:flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform z-50">
          {isSidebarOpen ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}
        </button>

        <nav className="flex-1 space-y-1.5 overflow-y-auto custom-scrollbar">
          <SidebarLink to="/call-center" icon={LayoutDashboard} label="لوحة التحكم" />
          <SidebarLink to="/call-center/pos" icon={ShoppingCart} label="نقطة البيع" />
          <SidebarLink to="/call-center/crm" icon={Database} label="قاعدة البيانات" />
          <SidebarLink to="/call-center/search" icon={Search} label="بحث بالهاتف" />
          <SidebarLink to="/call-center/complaints" icon={MessageSquareWarning} label="الشكاوى" />
          <SidebarLink to="/call-center/occasions" icon={Gift} label="المناسبات" />
          <SidebarLink to="/call-center/top-customers" icon={Star} label="أفضل العملاء" />
        ...
        <div className="h-full flex flex-col">
          <header className="lg:hidden p-4 flex items-center justify-between border-b border-white/5 bg-slate-900/50">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-cyan-600 rounded-lg flex items-center justify-center text-white font-bold shadow-lg shadow-cyan-900/20">
                <Headphones size={18} />
              </div>
