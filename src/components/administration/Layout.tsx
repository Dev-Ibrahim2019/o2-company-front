...
const NAV = [
  ...
  { to: "/admin/financial-invoices", icon: Receipt, label: "فواتير المبيعات", permission: PERMISSIONS.MANAGE_INVOICES, indent: true, group: "accounting" },
  ...
  { to: "/admin/audit", icon: FileText, label: "سجل التدقيق", permission: PERMISSIONS.VIEW_AUDIT_LOG },
...
export const AdminLayout: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  /* ── مكوّن رابط Sidebar ── */
  const SidebarLink = ({ to, icon: Icon, label, indent, exact, permission }: {
    to: string; icon: React.ElementType; label: string; indent?: boolean; exact?: boolean; permission?: string;
  }) => {
    const link = (
      <NavLink
        to={to}
  ...
  const SidebarGroup = ({ icon: Icon, label, groupKey, permission, children: groupChildren }: {
    icon: React.ElementType; label: string; groupKey: string; permission?: string; children: React.ReactNode;
  }) => {
    ...
    const group = (
      <div className="space-y-1">
        ...
        >
          ...
          {!collapsed && (
            <>
              <span className="font-semibold text-sm whitespace-nowrap overflow-hidden flex-1 text-right">{label}</span>
    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden text-slate-100 font-['Tajawal']" dir="rtl">
      ...
      <aside className={`fixed top-0 right-0 h-full bg-slate-900 border-l border-white/5 flex flex-col p-4 shadow-2xl transition-all duration-300 z-50 ${isSidebarOpen ? "w-64 translate-x-0" : "w-64 translate-x-full lg:w-20 lg:translate-x-0"}`}>
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto custom-scrollbar">
          {/* لوحة المعلومات */}
          <SidebarLink to="/admin/dashboard" icon={LayoutDashboard} label="لوحة المعلومات" exact />

          {/* إدارة الأفرع */}
          <SidebarLink to="/admin/branches" icon={Building2} label="إدارة الأفرع" permission={PERMISSIONS.MANAGE_BRANCHES} />

          {/* إدارة الأقسام */}
          <SidebarLink to="/admin/departments" icon={Layers} label="إدارة الأقسام" permission={PERMISSIONS.MANAGE_DEPARTMENTS} />

          {/* إدارة الأصناف */}
          <SidebarGroup icon={Package} label="إدارة الأصناف" groupKey="items" permission={PERMISSIONS.MANAGE_ITEMS}>
            <SidebarLink to="/admin/item-tree" icon={ListTree} label="شجرة الأصناف" indent />
            <SidebarLink to="/admin/items-index" icon={Table2} label="فهرس الأصناف" indent />
          </SidebarGroup>
          ...
          <SidebarGroup icon={BookOpen} label="المحاسبة والمالية" groupKey="accounting" permission={PERMISSIONS.VIEW_ACCOUNTING}>
            ...
            <SidebarLink to="/admin/financial-invoices" icon={Receipt} label="فواتير المبيعات" indent permission={PERMISSIONS.MANAGE_INVOICES} />
          ...
          {/* باقي الروابط */}
          <SidebarLink to="/admin/reports" icon={FileText} label="مركز التقارير" permission={PERMISSIONS.VIEW_REPORTS} />
          <SidebarLink to="/admin/audit" icon={FileText} label="سجل التدقيق" permission={PERMISSIONS.VIEW_AUDIT_LOG} />
          <SidebarLink to="/admin/archive" icon={Archive} label="أرشيف العمليات" permission={PERMISSIONS.VIEW_ARCHIVE} />
          <SidebarLink to="/admin/settings" icon={Settings} label="الإعدادات العامة" permission={PERMISSIONS.MANAGE_SETTINGS} />
          <SidebarLink to="/admin/orgstructure" icon={Building2} label="الهيكل التنظيمي" permission={PERMISSIONS.MANAGE_EMPLOYEES} />
