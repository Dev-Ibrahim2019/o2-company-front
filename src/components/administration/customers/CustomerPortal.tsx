    ...
    return (
        <div className="space-y-4">
            ...
            {loading ? (
                ...
                <>
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="bg-slate-950 border border-white/5 rounded-xl p-3">
                            ...
                            <p className="text-sm font-black font-mono text-rose-400">₪{money(statement.total_debit)}</p>
                        </div>
                        <div className="bg-slate-950 border border-white/5 rounded-xl p-3">
                            <p className="text-[10px] text-slate-500">إجمالي الدائن</p>
                            <p className="text-sm font-black font-mono text-emerald-400">₪{money(statement.total_credit)}</p>
                        </div>
                    </div>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {statement.lines.map((line: any, i: number) => (
                                        <tr key={i} className="hover:bg-white/[0.02]">
                                            ...
                                            <td className="px-4 py-3 text-rose-400 font-mono">{line.debit > 0 ? money(line.debit) : "—"}</td>
                                            <td className="px-4 py-3 text-emerald-400 font-mono">{line.credit > 0 ? money(line.credit) : "—"}</td>
                                            <td className={`px-4 py-3 font-mono font-bold ${line.balance > 0 ? "text-rose-400" : "text-emerald-400"}`}>₪{money(line.balance)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="flex items-center justify-center h-48 text-slate-500 font-bold">اختر الفترة ثم اضغط بحث</div>
            )}
        </div>
    );
};
